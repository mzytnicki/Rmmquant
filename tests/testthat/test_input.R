...
test_that("Running two SAM files test", {
    gtfFile     <- file.path(dir, "test.gtf")
    samFile     <- file.path(dir, "test.sam")
    object      <- RmmquantRun(gtfFile, c(samFile, samFile))
    table       <- counts(object)
    m           <- matrix(c(1, 1), 1, 2)
...
test_that("Running genomic ranges test", {
    samFile     <- file.path(dir, "test.sam")
    gr          <- GRanges(seqnames="chr1", ranges=IRanges(1000, width=3000, names="geneA"), strand="+")
    object      <- RmmquantRun(genomicRanges=gr, readsFiles=c(samFile))
    table       <- counts(object)
    m           <- matrix(1, 1, 1)
    dimnames(m) <- list(c("geneA"), c("test"))
    expect_equal(table, m)
...
test_that("Running genomic ranges list test", {
    ...
    gr2         <- GRanges(seqnames="chr2", ranges=IRanges(c(1000, 3000), width=1001, names="geneB"), strand="+")
    grl         <- GRangesList("geneA"=gr1, "geneB"=gr2)
    object      <- RmmquantRun(genomicRangesList=grl, readsFiles=c(samFile))
    table       <- counts(object)
    m           <- matrix(1, 1, 1)
    dimnames(m) <- list(c("geneA"), c("test"))
    expect_equal(table, m)
