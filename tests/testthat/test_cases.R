...
test_that("Running two chromosomes test", {
    ...
    object      <- RmmquantRun(genomicRangesList=grl, readsFiles=samFile)
    table       <- counts(object)
    m           <- matrix(c(1, 1), 2, 1)
...
test_that("Running antisense forward test", {
    ...
    grl         <- GRangesList("geneA"=gr1, "geneB"=gr2)
    samFile     <- file.path(dir, "test1.sam")
    object      <- RmmquantRun(genomicRangesList=grl, readsFiles=samFile, strands="F")
    table       <- counts(object)
    m           <- matrix(c(1), 1, 1)
...
test_that("Running antisense reverse test", {
    ...
    grl         <- GRangesList("geneA"=gr1, "geneB"=gr2)
    samFile     <- file.path(dir, "test1.sam")
    object      <- RmmquantRun(genomicRangesList=grl, readsFiles=samFile, strands="R")
    table       <- counts(object)
    m           <- matrix(c(1), 1, 1)
