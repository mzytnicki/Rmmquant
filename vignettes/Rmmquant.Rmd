...
[this article](https://www.nature.com/articles/mp2013170) [@data],
where the brain transcriptomes of patients diagnosed with bipolar disorder
are compared to healthy ones.

We have mapped the reads with STAR, selected the region chr21:42M-45M, and
down-sampled to 10%, to have small BAM files.

The reference genome is hg38.

```{r, bamfiles}
coldata           <- read.csv(file.path(dir, "data.csv"), row.names=1)
coldata$condition <- factor(coldata$condition, levels = c("control", "BD"))
bamFiles          <- file.path(dir, coldata$fileName)
```
...
```{r, annotation}
allGR <- genes(TxDb.Hsapiens.UCSC.hg38.knownGene, filter=list(tx_chrom="chr21"))
gr    <- allGR[start(allGR) >= 42000000 & end(allGR) <= 45000000]
```
...
Notice that some Entrez IDs have not Ensembl counterparts.
```{r, ensembl}
mcols(gr)$ensembl <- unlist(as.list(org.Hs.egENSEMBL[mcols(gr)$gene_id]))
gr                <- gr[! is.na(mcols(gr)$ensembl), ]
names(gr)         <- mcols(gr)$ensembl
RmmquantRun(genomicRanges=gr, readsFiles=bamFiles, sorts=TRUE, nThreads=6)
```
...
Here, we used the annotation extracted from Ensembl, GRCh38.76.

```{r, deseq2}
gtfFile  <- file.path(dir, "Homo_sapiens.gtf")
output   <- RmmquantRun(gtfFile, bamFiles, sorts=TRUE, nThreads=6)
